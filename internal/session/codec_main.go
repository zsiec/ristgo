...
// needs its own codec so the pure-SRP use_key_as_passphrase mode can key each path's
// feedback channel to that path's own session key K (the media stays cleartext), and so
// each path's GRE sequence is independent.
