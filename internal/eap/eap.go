...
// framing and state machine, byte-exact with libRIST v0.2.18-rc1
// (src/proto/eap.c, src/proto/eap.h). EAP frames ride inside a GRE EAPOL frame
...
//
// # Wire framing (eap.h, eap.c)
//
// Three nested headers precede every EAP-SRP body:
//
//	EAPOL header (4 bytes, struct eapol_hdr, eap.h:24-28):
//	    eapversion (1)  -- 2, or 3 for RFC 5054 PAD-compliant hashing
//	    eaptype    (1)  -- EAP=0, START=1, LOGOFF=2 (eap.h:20-22)
//	    length     (2)  -- big-endian; the EAP packet length (eap.c:162 sets
//	                       it equal to eap_hdr->length)
//	EAP header (4 bytes, struct eap_hdr, eap.h:35-39):
...
//	                       plus its body), eap.c:774-776 require length == len
//	EAP-SRP subtype header (2 bytes, struct eap_srp_hdr, eap.h:62-65):
//	    type       (1)  -- EAP_TYPE_SRP_SHA1 = 19 (eap.h:53)
...
// (eap.c:207-210 RESPONSE, eap.c:815-816 REQUEST).
//
// # SRP subtypes and bodies
//
// The subtype number's meaning depends on the EAP code (REQUEST vs RESPONSE);
// libRIST reuses values 1 and 2 in both directions (eap.h:48-56):
//
...
//	    [g | N]   (eap.c:498-522). For the default 2048-bit group gen_len==0
//	    and g/N are omitted (eap.c:269, :505-508).
...
//	    flags(4) | M1   (eap.c:293-307); the 4-byte flags word is zero in this
//	    implementation (M1 is the 32-byte client proof).
//	SERVER_VALIDATOR  (REQUEST, subtype 3, server->client): SRP-hdr |
//	    flags(4) | M2   (eap.c:602-614); flags zero, M2 the 32-byte proof.
//
// # State machine
//
// Two roles drive the same message exchange (eap.c process_eap_* handlers):
//
...
//
// EAPOL frames are never encrypted, even under a PSK (gre.c:25 excludes EAPOL
...
var (
	...
	// the EAPOL/EAP/SRP headers or a field a header announces (the eap.c
	...
	// inconsistent with the buffer (eap.c:775, :888 length validation).
	...
	// the role does not expect it (the eap.c EAP_UNEXPECTEDREQUEST /
	...
	// reports no verifier for the requested username (eap.c:487 found==false).
	...
	// failed: the peer's proof did not verify (eap.c:344-361 server M2 fail,
	// :569-595 client M1 fail).
	...
	// password exceeds 255 bytes (libRIST bounds both at 1..255, eap.c:1119, :1169).
	...
	// default and TR-06-2 use the default group exclusively (eap.c:269,
	// :490-491 default_ng); internal/srp exposes only that group, so an explicit
	// group is rejected.
	ErrUnsupportedGroup = errors.New("rist: eap: non-default SRP group unsupported")
)

// EAPOL types (eap.h:20-22, 802.1X-2010 §11).
const (
...
// EAP codes (eap.h:30-33, RFC 3748).
const (
...
// EAP method types (eap.h:43-53).
const (
	eapTypeIdentity uint8 = 1
	eapTypeSRPSHA1  uint8 = 19
)

// EAP-SRP subtypes (eap.h:48-56). Values 1 and 2 are reused across the
...
// 5054 PAD-compliant SRP hashing (eap.c:213 ctx->eapversion3 ? 3 : 2). libRIST
// 0.2.16+ always uses 3 for new contexts (eap.c:1149, :1189).
const eapVersion3 uint8 = 3

// Header sizes.
const (
	eapolHdrSize = 4 // struct eapol_hdr (eap.h:24-28)
	eapHdrSize   = 4 // struct eap_hdr   (eap.h:35-39)
	srpHdrSize   = 2 // struct eap_srp_hdr (eap.h:62-65)

	// hdrsOffset is EAPOL_EAP_HDRS_OFFSET (eap.h:41): the EAPOL header plus
	// the EAP header, i.e. the offset of an EAP body within an EAPOL frame.
	hdrsOffset = eapolHdrSize + eapHdrSize

	// proofLen is the SHA-256 proof size for M1/M2 (DIGEST_LENGTH, eap.c:33).
	proofLen = 32

	// validatorFlagsLen is the 4-byte flags word that prefixes M1 in
	// CLIENT_VALIDATOR and M2 in SERVER_VALIDATOR (eap.c:298, :606).
	validatorFlagsLen = 4
)
...
// (_librist_proto_eap_start, eap.c:824-835): version 3, type START, length 0.
...
// buf, returning the extended slice. It mirrors send_eapol_pkt (eap.c:151-183):
// the EAPOL length and EAP length fields both carry the EAP packet length
// (eap.c:162). It never panics; an unknown kind appends nothing and returns buf
// unchanged.
func (f Frame) AppendTo(buf []byte) []byte {
	...
	// EAP header (eap.c:828-832: eapol.length = 0, struct eapol_hdr only).
	switch f.Kind {
...
func (f Frame) encodeBody() []byte {
	...
	case KindClientKey:
		...
		// (eap.c:276-278): SRP header then A.
		out := []byte{eapTypeSRPSHA1, srpSubtypeChallenge}
		return append(out, f.Public...)
	case KindServerKey:
		// SERVER_KEY REQUEST (eap.c:550-552): SRP header then B.
		out := []byte{eapTypeSRPSHA1, srpSubtypeServerKey}
		return append(out, f.Public...)
	case KindClientValidator:
		// CLIENT_VALIDATOR rides on subtype SERVER_KEY in the RESPONSE
		// direction (eap.c:300-307): SRP header, 4-byte flags, M1.
		out := []byte{eapTypeSRPSHA1, srpSubtypeServerKey}
		out = append(out, f.Flags[:]...)
		return append(out, f.Proof...)
	case KindServerValidator:
		// SERVER_VALIDATOR REQUEST (eap.c:604-612): SRP header, flags, M2.
	...
	case KindSuccess:
		...
		// (eap.c:333-339). Emit two zero bytes to match the wire.
		return []byte{0, 0}
	case KindFailure:
		// FAILURE carries no body (eap.c:400, :592).
		return nil
	default:
		return nil
	}
...
func parseEAP(version uint8, eapPkt []byte) (Frame, error) {
	...
	// eap.c:775-776: the EAP header length must equal the packet length.
...
func parseSRP(f Frame, code, subtype uint8, payload []byte) (Frame, error) {
	...
	case subtype == srpSubtypeServerVal:
		...
		// proof (process_eap_response_srp_server_validator, eap.c:617-634;
		// dispatched at eap.c:734-735); we surface it as a bodyless
		// server-validator so the host can complete the handshake on it.
...
// parseChallenge decodes the CHALLENGE TLVs (eap.c:226-268): name_len|name|
...
func parseChallenge(f Frame, p []byte) (Frame, error) {
	...
	off += nameLen // name ignored (eap.c:233)
	...
	if genLen != 0 {
		...
		off += genLen
		f.GenN = cloneBytes(p[off:]) // N runs to the end (eap.c:264-265)
	}
	return f, nil
}
...
// unknown (eap.c:487 found). The returned slices are treated as read-only.
...
// single-user mode (eap.c:1028-1084). It is the common host configuration.
