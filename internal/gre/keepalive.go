...
// to hold the fixed keep-alive body (the
...
// The first capability octet (capabilities1) carries N, L,
...
// mirror libRIST's rist_keepalive_info booleans.
...
// reduced-overhead support (V). The pair-split bit (L) is set
...
// Keepalive is a parsed RIST keep-alive message body (plus the
...
type Keepalive struct {
	...
	// Caps are the negotiated capability bits.
	Caps Capabilities

	// HasAdvExt reports whether the optional 4-byte Advanced-profile
	// extended-capability block is present.
	HasAdvExt bool
	...
	// JSON is the optional trailing JSON message payload. It is
...
// extended-capability block and JSON payload. It writes only
...
// heuristic and its sender never attaches a JSON payload.
...
// mirrors _librist_proto_gre_parse_keepalive and never panics
